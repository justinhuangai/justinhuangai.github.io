...
Seq2Seq 的底层矛盾很朴素：输入和输出长度都不固定，传统翻译流水线能处理，却很难端到端优化。[《Sequence to Sequence Learning with Neural Networks》](/papers/1409.3215v3.pdf) 把问题改写成两个接口：一个网络读完整段输入，另一个网络逐步生成输出。
