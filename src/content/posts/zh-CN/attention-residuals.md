---
title: "《Attention Residuals》：让残差连接也注意力化"
...
这里最值得注意的是 GPQA、Math、HumanEval 这种多步推理或程序生成任务涨幅更大。报告作者的解释是：如果后层能更有选择地回收前层表示，那么需要组合式推理的任务会更受益。这个解释是说得通的。
...
这正是这份报告有意思的地方。你从正文、消融和系统设计里能更清楚地看到他们的真实取舍：不是盲目追求最低 loss，而是在追求“足够强，同时真能训起来”。
...
而这正是报告讨论部分厉害的地方。作者把一堆残差变体统一进了一个 `depth mixing matrix` 的视角里，进一步指出：
...
几年后 AttnRes 会不会像 PreNorm 一样成为默认配置还不确定，但这篇技术报告已经把残差连接重新变成了一个值得被思考、被设计、被优化的对象。
