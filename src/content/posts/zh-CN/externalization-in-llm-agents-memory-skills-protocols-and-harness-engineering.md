...
哪个模型更聪明，哪个推理更强，哪个上下文窗口更长，这些当然重要。但这篇 [《Externalization in LLM Agents: A Unified Review of Memory, Skills, Protocols and Harness Engineering》](/papers/2604.08224v1.pdf) 提醒我，Agent 的另一条主线不在模型内部，而在模型外部。
...
这也是为什么我觉得 skills 是 Agent 工程里最容易被低估的一层。它不炫，但它直接降低方差。
...
对开发者来说，这个视角很实用。下次看到一个 Agent 系统，不要只问它用了哪个模型，也要问：
