...
Elixir 在這張表裡排名這麼高，當然不能直接說明「大模型最擅長 Elixir」。前面那些變量都是真的：benchmark 的生成流程會影響結果，翻譯路徑會影響結果，過濾器在低資源語言上的能力也會影響結果。

但即便把這些因素都算進去，Elixir 這一欄還是值得單獨拿出來看。

因為問題到最後已經不是「這個分數該怎麼解釋」，而是：為什麼偏偏是 Elixir？

如果把 benchmark 的生成流程、翻譯路徑、難度過濾這些條件都擺在桌面上之後，它還是這麼顯眼，那 Elixir 至少值得繼續往下追。

不是為了急著證明「大模型擅長 Elixir」，而是因為它背後可能確實有一套更值得研究的東西。

也許是 Elixir 的並發模型，也許是抽象方式，也許是任務表達和語言特性，和今天的大模型運行機制剛好有著某種更深的關係。

OpenAI 最近開源的 [Symphony](https://github.com/openai/symphony) 專案，也是基於 Elixir 實現的。

如果對這個方向感興趣，可以接著閱讀這篇 [《為什麼 Elixir 是最適合 AI 的程式語言》](/zh-TW/translations/why-elixir-is-the-best-language-for-ai/)。
